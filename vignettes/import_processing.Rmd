...
In cases where there is no load data saved in the metabolic cart's file, or `get_protocol()` turns wrong, a protocol can be manually generated via `set_protocol_manual` and applied to the interpolated data by `add_protocol()`.
