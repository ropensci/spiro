...
Alternatively to providing every single load step, a protocol can be defined by segments with `set_protocol`. Pre-defined segment types such as `pre`, `wu` and `steps` can be pasted into the function in the desired order. The following graph illustrates an example:
