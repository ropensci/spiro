...
We highly recommended to import only raw breath-by-breath data for several reasons:
 ...
 - If no data for single breaths is available, some functionality of summary functions (e.g. breath averaging for VO2max determination) is lost.
...
If you use a metabolic carts that measures, but does not output data on a breath-by-breath basis, read the manufacturer's instruction on how to export the raw data in such a way. Data from other systems (mixed chamber) can still be processed with the `spiro`-package, but protocol guesses and summary calculations have to be treated with caution.
...
* **CORTEX** (`"cortex"`): .xlsx, .xls or .xml files in German language
