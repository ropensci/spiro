...
To directly get started, paste the name of a breath-by-breath raw data file coming from a supported device type into the `spiro()` function. This will return a `data.frame` containing all relevant data for each second of testing, ready for summarising or plotting.
...
`spiro_interpolate` performs a linear interpolation on the imported data to obtain values for ever second.
```{r interpolate}
...
To archieve the full functionality of data summary and plotting with the spiro-package, a testing protocol needs to be attached to the imported data. The parameters of exercise protocols in `spiro` are the following:
...
To avoid specifying every single protocol parameter to `spiro_protocol()` there a wrapper function for common types of exercise test:
...
The imported and interpolated gas exchange data, as well as the protocol load data allows the calculation of additional variables with `spiro_add`
...
For calculating parameters relative to body weight, `spiro_add()` will use the weight found in the original data file's meta data. If for some reason no or the wrong weight was present in the raw data file, `weight` can be manually supllied.
...
`hr_add()` will import the heart rate data from a *tcx file via `hr_import()` and attach it to the existing data with `hr_attach()`. Note that you can manually supply the starting point of the heart rate recording in relation to the gas exchange measures recording with `hr_offset`.
