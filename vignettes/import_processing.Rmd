...
ggplot() +
  ...
  annotate("text", x = 0.53, y = 0.46, label = "add_protocol()", colour = "grey", hjust = "left") +
  annotate("text", x = 0.53, y = 0.38, label = "add_weight()", colour = "grey", hjust = "left") +
...
data_with_ptcl <- add_protocol(data = data_int, protocol = pt)
data_with_ptcl
```
...
```{r spiro_add}
add_weight(data_with_ptcl, weight = NULL)
```
...
Some metabolic carts only offer complicated options for connecting them to heart rate monitors. If heart rate data was recorded by another kind of device (e.g. wrist watch), this data can be attached to the existing `data.frame` of gas exchange measures with `add_hr()`.
...
add_hr(data_rt, hr_file = hr_data, hr_offset = 0)
```

`add_hr()` will import the heart rate data from a .tcx file and attach it to the existing data set. Note that you can manually supply the starting point of the heart rate recording in relation to the gas exchange measures recording with `hr_offset`.
