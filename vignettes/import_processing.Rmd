...
Alternatively to providing every single load step, a protocol can be defined by segments with `set_protocol`. Pre-defined segment types such as `pre`, `wu` and `step` can be pasted into the function in the desired order. The following graph illustrates an example:
...
ggplot() +
  ...
  annotate("text", x = c(0.03,0.05,0.05,0.05,0.03), y = c(0.97,0.90,0.83,0.76,0.69), 
           label = c(
             ...
             "step(duration, load, increment, count, rest)",
              ")"), 
           hjust = "left", vjust = "top",
           colour = c("black", "#d55e00", "#009e73", dsblue, "black"),
           size = 4.5) +
  scale_x_continuous(limits = c(-0.02,1), expand = expansion(0,0)) +
  scale_y_continuous(limits = c(-0.15,1), expand = expansion(0,0)) +
...
```{r}
set_protocol(pre(60), wu(300,80), step(180,100,25,6,30))
```
