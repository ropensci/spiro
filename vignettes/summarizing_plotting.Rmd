...
For some types of exercise tests it may be preferable to get maximal values of the measured parameters. `spiro_max()` calculates these after smoothing the data.

```{r spiro_max}
spiro_max(gxt_data, smooth = 30)
```
The `smooth` argument controls the interval and method for the data smoothing. Smoothing happens by the calculation of a rolling average either over a fixed time interval (e.g. `smooth = 30` for 30 seconds), or over a fixed number of breaths (e.g. `smooth = "30b"` for 30 breaths). Per default the smoothing will not apply to the heart rate values, but you can enable this behavior with `hr_smooth = TRUE`.
...
```{r spiro_plot, fig.width = 10, fig.height = 8}
# load example data
...
```{r spiro_plot-style, fig.width = 10, fig.height = 8}
