...
interest when analysing spiroergometric data. `spiro_plot_VO2()` visualises the rolling average for the interpolated relative oxygen uptake data and the underlying protocol load.

The length of the computational interval for the rolling average can be changed by setting the `smooth` argument.

```{r spiro_plot_VO2, warning=FALSE}
spiro_plot_VO2(gxt_data, smooth = 15)
```
