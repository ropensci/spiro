...
The length of the computational interval in seconds can be modified by setting the `interval` argument. If the interval exceed the length of any step, it will be shortened in these cases and a note will be displayed.
...
For constant load tests `spiro_glance()` will behave different, as not the maximal values, but the overall average is usually useful to calculate. Therefore the mean parameter values for all steps of the constant load will be displayed.

## Plotting the data

To plot exercise test data retrieved by `spiro()` it is recommend to use specific visualisation packages such as `ggplot2`, which give a variety of possible applications.
